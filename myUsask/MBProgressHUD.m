...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		label.shadowColor = [UIColor colorWithRed:0.00 green:0.00 blue:0.00 alpha:0.6];
...
- (void)layoutSubviews {
    ...
    indFrame.origin.x = floorf((frame.size.width - indFrame.size.width) / 2) + self.xOffset + 1;
    indFrame.origin.y = floorf((frame.size.height - indFrame.size.height) / 2) + self.yOffset - 3.0;
    ...
    if (nil != self.labelText) {
        ...
        CGRect lFrame = CGRectMake(floorf((frame.size.width - lWidth) / 2) + xOffset + 1,
                                   floorf(indFrame.origin.y + indFrame.size.height + PADDING + 6.0),
                                   lWidth, lHeight);
        label.frame = lFrame;
